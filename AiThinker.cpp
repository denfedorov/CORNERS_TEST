...
void AiThinker::registerAiMove(int sx, int sy, int x, int y) {
	Move nm(sx, sy, x, y);
	ai_moves[ai_moves_cnt++] = nm;
}

void AiThinker::generateAiMoves(void) {
	ai_moves_cnt = 0;
	for (int y = 0; y < 8; y++) {
		for (int x = 0; x < 8; x++) {
			if (init_desk.field[y][x].type == AI_CELL) {
				if (init_desk.canMove(x + 1, y,false)) {
					registerAiMove(x, y, x + 1, y);
				}
				if (init_desk.canMove(x, y + 1,false)) {
...
AiThinker::AiThinker(Desk* init) {
	init_desk = *init;
}

bool AiThinker::FindPass(Desk& d, int& passlen, int temp_pathlen, int fromx, int fromy, int tox, int toy) {

	if (!init_desk.canMove(fromx, fromy,false)) {
		return false;
	}
...
double AiThinker::getAiRang(Desk& desk) {
	double waysfound = 0;
	double passlen = 1000;
...
void AiThinker::calcRangOfMoves(void) {
...
void AiThinker::sortMovesByRank(void) {
	for (int i = 0; i < ai_moves_cnt; i++) {
...
bool AiThinker::OptimizeAiTurn(void) {
	...
	for (int i = 0; i < ai_moves_cnt; i++) {
		...
		if (cm->to_x > 4 && cm->to_y > 4) {
			...
			if (tdesk.canMove(cm->to_x, cm->to_y,false)) {
...
void AiThinker::makeBestMove() {
	desigion_desk = init_desk;
	for (int i = 0; i < ai_moves_cnt; i++) {
		Move* tm = &ai_moves[i];
		if (desigion_desk.canMove(tm->to_x, tm->to_y,false)) {
